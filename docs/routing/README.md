...
- `/{lang}/install/redirect?...` -> 모바일에서 감지된 플랫폼 스토어 URL로 `redirect`, 데스크톱에서는 `/{lang}/install`로 `redirect`
...
- `/{lang}/install/redirect` (`source`, `dest`, `lang`, `utm_*`, `variant` query 지원, redirect-only)
