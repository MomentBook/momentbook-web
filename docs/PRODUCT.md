...
- 공개 상세 viewer payload에 `localizedContent`가 있으면 현재 route locale과 일치하는 entry(`title`, `description`, `hashtags`, cluster `impression`)를 우선 사용하고, 없으면 원문 필드로 fallback한다.
...
- `/{lang}/journeys/[journeyId]`와 `/{lang}/journeys/[journeyId]/moments/[clusterId]`는 viewer payload의 `localizedContent`가 현재 locale과 일치할 때 localized title/description/hashtags 및 cluster impression을 SEO metadata/structured data에 우선 반영하고, 본문 제목/설명도 localized title/description을 우선 사용한다.
