...
현재 query 동작:
- `/{lang}/journeys`는 첫 batch를 server-rendered로 노출한 뒤, 같은 route 안에서 `sort=discovery` + offset page 기반 `load more`를 사용한다. legacy `?page=` 진입은 base route로 redirect한다.
...
- `GET /v2/journeys/public`는 discovery feed endpoint로 사용한다. 웹의 public/discovery feed 요청은 approved-only 계약을 명시하기 위해 항상 `reviewStatus=APPROVED`를 포함한다. `/{lang}/journeys`는 `sort=discovery` + offset page 기반 batch를 사용한다. 홈의 recent journeys, sitemap 등 최신순 의미가 명확한 경로는 `sort=recent`를 유지한다. cursor pagination은 현재 `sort=recent` short-feed 경로에서만 허용되며 `sort=discovery`와 함께 사용하지 않는다. 작성자별 목록은 deprecated `userId` query를 쓰지 않고 `GET /v2/users/public/:userId/journeys`를 사용한다.
...
- `/{lang}/journeys`는 단일 canonical/alternates를 유지하는 index/follow discovery route다. 추가 discovery batch는 client-side offset page 요청으로 이어지며 별도 공개 URL을 만들지 않는다.
