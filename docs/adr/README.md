...
- Status: Accepted
- Summary: route-group root layouts + `app/(localized)/[lang]` + `proxy.ts` 기반 언어 prefix 라우팅, SSR `html lang`, hreflang alternates
...
- Date: 2025-01-04 (updated: 2026-03-30)
- Status: Accepted
- Summary: 정적 중심 + route별 revalidate(60/3600) + localized time SSR/hydration 분리 + route-group root layouts로 SSR `html lang` 유지(`/{lang}/install` 예외)
...
- Date: 2026-01-29 (updated: 2026-03-28)
- Status: Accepted
- Summary: 공개 콘텐츠 API 우선, lang query 기반 localized public fetch, web detail viewer endpoint 사용, list preview server thumbnail 우선, host 후보 fallback fetch, generated API 타입 계약
