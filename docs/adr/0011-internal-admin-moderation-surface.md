...
5. 최초 관리자 기능은 공개 여정 심사 큐에만 한정한다.
   ...
   - `reviews`는 mock preview dataset을 data-table + preview panel로 렌더링한다.
   - moderation write는 별도 tab 없이 `reviews`의 status update form에서 known `publicId`에 대해 `PATCH /v2/admin/journeys/publish/:publicId/review`만 연결한다.
   - backend에 admin queue/list read API가 없으므로 실제 pending 목록은 이 표면에서 backend read API로 조회하지 않는다.

## Rationale

같은 프로젝트 안에 내부 운영 표면을 두면 배포와 코드 공유가 단순해진다.
overview와 reviews를 분리하면 운영 대시보드와 실제 심사 작업을 같은 표면 안에서 구분할 수 있다.
reviews는 mock preview table로 유지하되, 실제 review status mutation만 연결하면 server contract 변화에 대응하면서도 존재하지 않는 read surface를 추정 구현하지 않을 수 있다.
...
- admin queue/list read API가 없으므로 queue table은 mock data에 머물고, 운영자는 known `publicId`가 있을 때만 실제 moderation mutation을 실행할 수 있다.
