...
4. 관리자 인증은 공개 웹 auth 부활이 아니라 backend admin role을 사용하는 내부 로그인으로 제한한다.
   ...
   - env constraint: 이 흐름에 쓰이는 `NEXT_PUBLIC_API_BASE_URL`은 관리자 브라우저에서 실제로 도달 가능한 origin이어야 하며, 원격 접속 환경에서는 `127.0.0.1` loopback을 사용할 수 없다.
...
5. 최초 관리자 기능은 공개 여정 심사 큐에만 한정한다.
   ...
   - `/admin/login`
   - `/admin/reviews`
   - `overview`는 moderation summary, queue snapshot, session/access 상태만 보여주는 대시보드로 유지한다.
   - `reviews`는 `GET /v2/admin/journeys/publish`를 사용해 backend-backed published journey list를 렌더링한다.
   - backend admin read contract는 `page`/`limit`만 제공하므로, `pending`/`approved`/`rejected`/`all` 필터와 queue pagination snapshot은 웹이 fetched admin records를 기준으로 계산한다.
   - `/admin/reviews/[publicId]`는 list API에 포함된 contract-backed summary만 보여주고, full gallery/author-profile detail 같은 비계약 필드는 추정하지 않는다.
   - moderation write는 별도 tab 없이 `reviews`의 status update form에서 known `publicId`에 대해 `PATCH /v2/admin/journeys/publish/:publicId/review`를 연결한다.

## Rationale

같은 프로젝트 안에 내부 운영 표면을 두면 배포와 코드 공유가 단순해진다.
overview와 reviews를 분리하면 운영 대시보드와 실제 심사 작업을 같은 표면 안에서 구분할 수 있다.
backend-backed admin list를 사용하면 mock queue drift를 줄이면서 실제 moderation 대상 전체를 같은 표면에서 확인할 수 있다.
동시에 현재 read contract에 없는 full detail/gallery를 추정 구현하지 않고, 계약에 포함된 필드만 detail summary에 노출하면 동작과 문서를 함께 단순하게 유지할 수 있다.
...
- 운영자가 pending/rejected/hidden 상태를 포함한 실제 published journey 목록을 웹에서 확인할 수 있다.
...
- backend admin list contract가 status-specific counts/detail/gallery를 직접 주지 않으므로, 웹이 fetched admin records를 다시 집계해야 한다.
- detail route는 list endpoint에 포함된 최소 정보만 보여주므로, richer preview는 별도 backend read contract가 생기기 전까지 제공하지 않는다.
