...
- production browser는 서비스워커(`public-image-cache-sw.js`)를 등록해 MomentBook CDN/S3 원격 published image를 client-side cache storage에 `stale-while-revalidate`로 저장한다.
- 이미지 캐시는 원격 asset URL을 그대로 사용하며, Vercel image optimizer 또는 app route proxy를 추가하지 않는다.
...
또한 원격 published image를 브라우저 cache storage에 직접 저장하면 Vercel 비용을 늘리지 않고도 반복 방문/상세 이동 시 AWS 이미지 전송을 줄일 수 있다.
...
- ✅ 원격 published image를 브라우저 캐시에서 재사용해 반복 이미지 요청의 AWS 전송량을 줄인다

### Negative
- ⚠️ 페이지별 TTL이 다르므로 운영 이해가 필요
- ⚠️ 콘텐츠 업데이트가 즉시 반영되지 않을 수 있음
- ⚠️ 서비스워커 기반 이미지 캐시는 브라우저 지원 범위 안에서만 동작한다
...
- `components/PublicImageCacheRegistration.tsx`
- `app/(localized)/[lang]/(chrome)/(content)/photos/[photoId]/LocalizedDateTime.tsx`
